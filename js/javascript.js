$(document).ready(function(){	
	$('.ball5').on({
        'click': function(){
            var imgId = "#" + this.id            
            $(imgId).css("bottom") === "80px"? $(imgId).css("bottom", "53px"):$(imgId).css("bottom", "80px");
        }
    });
    ...
    $('.lower > img').on({
        'click': function (){
	        ...
	        var newBottomValue = (parseInt(baseBottomValue.slice(0, baseBottomValue.length-2)) + 36).toString() + "px";  
        	...
        	if ($(imgId).css("bottom") !== newBottomValue){
	            for (var i = numId; i < 5; i++) {   
	            	...
	            	newBottomValue = (parseInt(baseBottomValue.slice(0, baseBottomValue.length-2)) + 36).toString() + "px";   
